...
from .misc import cfg_with_default, count_param
import logging
...
class CatePrior(nn.Module):
    # just a wrapper for one category
    def __init__(self, cfg, model_id, use_double=True) -> None:
        ...
        logging.info(f"Model {self.model_id} at ep={field_loaded_ep})")
    ...
    def encode(self, x):
        input = x.double() if self.use_double else x
